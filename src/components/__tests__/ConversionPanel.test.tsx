...
describe('StateEliminationPanel — step-complete phase', () => {
  async function advanceToStepComplete() {
    ...
    while (keepGoing) {
      ...
      const completeBtn = screen.queryByRole('button', { name: /Complete Elimination/i })
      if (completeBtn) { keepGoing = false; break }
      if (autoBtn) await userEvent.click(autoBtn)
      if (nextBtn) await userEvent.click(nextBtn)
      if (!autoBtn && !nextBtn && !completeBtn) { keepGoing = false }
    }
  }
...
describe('StateEliminationPanel — finished phase', () => {
  async function runFullElimination() {
    ...
    while (iterations < 40) {
      ...
      if (autoPickBtn) { await userEvent.click(autoPickBtn); continue }

      const nextPathBtn = screen.queryByRole('button', { name: /Next Path/i })
      if (nextPathBtn) { await userEvent.click(nextPathBtn); continue }

      const autoCompleteBtn = screen.queryByRole('button', { name: /Auto-complete/i })
      if (autoCompleteBtn && !(autoCompleteBtn as HTMLButtonElement).disabled) {
        await userEvent.click(autoCompleteBtn); continue
      }

      const completeEliminationBtn = screen.queryByRole('button', { name: /Complete Elimination/i })
      if (completeEliminationBtn) { await userEvent.click(completeEliminationBtn); continue }
