...
import { thompsonReducer, initialThompsonState, type ThompsonState } from '../thompsonReducer'

describe('thompsonReducer — SET_REGEX', () => {
  it('updates the regex field and clears any existing error', () => {
    // Arrange
    const state: ThompsonState = { ...initialThompsonState, error: 'previous error' }
...
describe('thompsonReducer — START', () => {
  it('transitions to stepping phase and loads steps for a valid regex', () => {
    // Act
    const next = thompsonReducer(initialThompsonState, { type: 'START', payload: 'a' })
  ...
  it('keeps idle phase and sets error for an invalid regex', () => {
    // Act
    const next = thompsonReducer(initialThompsonState, { type: 'START', payload: '@bad' })
...
describe('thompsonReducer — SELECT_TEMPLATE', () => {
  it('marks isTemplateCorrect true when the chosen template matches the current step', () => {
    ...
    const started = thompsonReducer(initialThompsonState, { type: 'START', payload: 'a' })

    // Act
    const next = thompsonReducer(started, { type: 'SELECT_TEMPLATE', payload: 'symbol' })
  ...
  it('marks isTemplateCorrect false when the chosen template is wrong', () => {
    ...
    // Act
    const next = thompsonReducer(started, { type: 'SELECT_TEMPLATE', payload: 'union' })
  ...
  it('returns state unchanged when there is no current step (idle phase)', () => {
    ...
    const next = thompsonReducer(initialThompsonState, { type: 'SELECT_TEMPLATE', payload: 'symbol' })
...
describe('thompsonReducer — NEXT_STEP', () => {
  it('does not advance when the template has not been confirmed correct', () => {
    ...
    const started = thompsonReducer(initialThompsonState, { type: 'START', payload: 'ab' })
    const wrong = thompsonReducer(started, { type: 'SELECT_TEMPLATE', payload: 'union' })
  ...
  it('advances to the next step and resets template selection when correct', () => {
    ...
    const started = thompsonReducer(initialThompsonState, { type: 'START', payload: 'ab' })
    const correct = thompsonReducer(started, { type: 'SELECT_TEMPLATE', payload: 'symbol' })
  ...
  it('transitions to "complete" phase after confirming the final step', () => {
    ...
    let s = thompsonReducer(initialThompsonState, { type: 'START', payload: 'a' })
...
describe('thompsonReducer — RESET', () => {
  it('returns the initial state regardless of previous progress', () => {
    ...
    let s = thompsonReducer(initialThompsonState, { type: 'START', payload: 'a+b' })
    s = thompsonReducer(s, { type: 'SELECT_TEMPLATE', payload: 'symbol' })
