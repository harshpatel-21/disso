...
describe('nfaReducer — ADD_STATE', () => {
  it('appends the new state to the states array', () => {
    ...
    // Act
    const next = nfaReducer(initialNFAState, { type: 'ADD_STATE', payload: newState })
...
describe('nfaReducer — REMOVE_STATE', () => {
  it('removes the state with the given id', () => {
    ...
    const state: NFAState = {
      ...initialNFAState,
      nfa: { ...initialNFAState.nfa, states: [makeState('q0'), makeState('q1')] },
    }
...
describe('nfaReducer — ADD_TRANSITION', () => {
  it('appends the new transition to the transitions array', () => {
    ...
    // Act
    const next = nfaReducer(initialNFAState, { type: 'ADD_TRANSITION', payload: t })
...
describe('nfaReducer — REMOVE_TRANSITION', () => {
  it('removes the transition with the given id', () => {
    ...
    const state: NFAState = {
      ...initialNFAState,
      nfa: { ...initialNFAState.nfa, transitions: [makeTransition('t0', 'q0', 'q1')] },
    }

    // Act
    const next = nfaReducer(state, { type: 'REMOVE_TRANSITION', payload: 't0' })
  ...
  it('preserves selectedTransitionId when a different transition is removed', () => {
    ...
    const state: NFAState = {
      ...
      nfa: {
        ...initialNFAState.nfa,
        transitions: [makeTransition('t0', 'q0', 'q1'), makeTransition('t1', 'q1', 'q0')],
      },
      selectedTransitionId: 't0',
    }

    // Act
    const next = nfaReducer(state, { type: 'REMOVE_TRANSITION', payload: 't1' })
...
describe('nfaReducer — SET_ALPHABET', () => {
  it('replaces the entire alphabet', () => {
    // Act
    const next = nfaReducer(initialNFAState, { type: 'SET_ALPHABET', payload: ['a', 'b'] })
...
describe('nfaReducer — SET_APP_MODE', () => {
  it('updates appMode, resets nfaToRegexPhase to "input", and clears validation errors', () => {
    ...
    // Act
    const next = nfaReducer(state, { type: 'SET_APP_MODE', payload: 'regex-to-nfa' })
...
describe('nfaReducer — SELECT_STATE / SELECT_TRANSITION', () => {
  it('sets selectedStateId to the given id', () => {
    // Act
    const next = nfaReducer(initialNFAState, { type: 'SELECT_STATE', payload: 'q0' })
  ...
  it('sets selectedTransitionId to the given id', () => {
    // Act
    const next = nfaReducer(initialNFAState, { type: 'SELECT_TRANSITION', payload: 't0' })
...
describe('nfaReducer — SET_VALIDATION_ERRORS', () => {
  it('stores the provided validation errors', () => {
    ...
    // Act
    const next = nfaReducer(initialNFAState, { type: 'SET_VALIDATION_ERRORS', payload: errors })
