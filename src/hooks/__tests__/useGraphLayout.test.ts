...
function makeNFA(stateIds: string[], transitions: NFA['transitions'] = []): NFA {
  return {
...
describe('useGraphLayout — node mapping', () => {
  ...
  it('flags the state being eliminated via highlight.stateToRemove', () => {
    // Arrange
    const nfa = makeNFA(['q0', 'q1'])
    const highlight = { stateToRemove: 'q0', currentPath: null, highlightedR: null }
  ...
  it('flags predecessor and successor states from the current path', () => {
    ...
    const currentPath = { from: 'q0', to: 'q2', R1: 'a', R2: '∅', R3: 'b', R4: '∅', expectedResult: 'ab' }
  ...
  it('flags newly added Thompson states via thompsonHighlight', () => {
    ...
    // Act
    const { result } = renderHook(() => useGraphLayout(nfa, { thompsonHighlight }))
...
describe('useGraphLayout — edge mapping', () => {
  it('produces one edge per transition', () => {
    // Arrange
    const nfa = makeNFA(['q0', 'q1'], [{ id: 't0', source: 'q0', target: 'q1', symbol: 'a' }])
  ...
  it('marks self-loop edges as isSelfLoop', () => {
    // Arrange
    const nfa = makeNFA(['q0', 'q1'], [{ id: 't0', source: 'q0', target: 'q0', symbol: 'a' }])
  ...
  it('highlights R1 edge (predecessor → removed) when highlightedR is R1', () => {
    ...
    const currentPath = { from: 'q0', to: 'q2', R1: 'a', R2: '∅', R3: 'b', R4: '∅', expectedResult: 'ab' }
    const highlight = { stateToRemove: 'q1', currentPath, highlightedR: 'R1' as const }
  ...
  it('highlights R2 edge (self-loop on removed state) when highlightedR is R2', () => {
    ...
    const currentPath = { from: 'q0', to: 'q2', R1: 'a', R2: 'b', R3: 'c', R4: '∅', expectedResult: 'ab*c' }
    const highlight = { stateToRemove: 'q1', currentPath, highlightedR: 'R2' as const }
  ...
  it('highlights R3 edge (removed → successor) when highlightedR is R3', () => {
    ...
    const currentPath = { from: 'q0', to: 'q2', R1: 'a', R2: '∅', R3: 'c', R4: '∅', expectedResult: 'ac' }
    const highlight = { stateToRemove: 'q1', currentPath, highlightedR: 'R3' as const }
  ...
  it('highlights R4 edge (predecessor → successor direct) when highlightedR is R4', () => {
    ...
    const currentPath = { from: 'q0', to: 'q2', R1: 'a', R2: '∅', R3: 'b', R4: 'd', expectedResult: 'd+ab' }
    const highlight = { stateToRemove: 'q1', currentPath, highlightedR: 'R4' as const }
